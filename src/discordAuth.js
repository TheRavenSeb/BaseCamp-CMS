...
if(process.env.NODE_ENV === 'production'){
  uri = 'https://arcane-magica.com/discord-oauth-callback'
}
else{
    uri = 'https://localhost:5000/discord-oauth-callback'
    }
...
function getOAuthUrl() {
  ...
  url.searchParams.set('client_id', process.env.D_ID);
  url.searchParams.set('redirect_uri', 'https://arcane-magica.com/discord-oauth-callback');
...
async function getOAuthTokens(code) {
  ...
  const body = new URLSearchParams({
    client_id: process.env.D_ID,
    client_secret: process.env.D_SECRET,
...
async function pushMetadata(userId, tokens, metadata) {
  ...
  const url = `https://discord.com/api/v10/users/@me/applications/${process.env.D_ID}/role-connection`;
  const accessToken = await getAccessToken(userId, tokens);
  console.log('got token');
  const body = {
    platform_name: 'Arcane Magica',
    metadata,
  };
  const response = await fetch(url, {
    method: 'PUT',
